...
"""rfsim -- the executable front of the interactive Rotating Frame tool.

This file is deliberately almost empty. The command's body is
`rotating_frame.cli.rfsim`, because the tool is reached in two ways that
...
- through the `rfsim` console script that `pip install` creates,
  which calls `rotating_frame.cli.rfsim.console_main`.
...
Run `rfsim --help` for the usage.
"""

import sys
from pathlib import Path

# The library is src/rotating_frame/, beside this scripts/ directory. With
...
from rotating_frame.cli.rfsim import main, record_command     # noqa: E402
