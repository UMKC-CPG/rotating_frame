...
is part of the Rotating Frame teaching tool.
"""
...
RC_ENVIRONMENT_VARIABLE = 'ROTATING_FRAME_RC'
...
def example_files():
    ...
    directory = Path(str(resources.files('rotating_frame.examples')))
...
def copy_without_overwriting(sources, directory, command_name='rfsim'):
    ...
    try:
        ...
        print(f'cannot write in {directory} ({problem.strerror}). Choose '
              ...
              f'    {command_name} --examples ~/rotating_frame-runs',
              file=sys.stderr)
        return 1
    return 0
