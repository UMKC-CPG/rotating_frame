...
> **Session setup:** run `/rename ROTATING_FRAME` (and a `/color`) at the
...
src/
  rotating_frame/        The importable library, by concern, plus:
    cli/          Bodies of the commands; support.py is inherited
...
row 0 lists them: the fronts in `src/scripts/`, `cli/support.py`,
`render/offscreen.py`, the `defaults/` and `examples/` packages'
`__init__.py`, `tests/conftest.py`, and the installed-copy and
offscreen tests. They are governed by the suite's `dev/PSEUDOCODE.md`
section 4. A change to one of them is made in the suite's skeleton
first and carried here; do not let this copy drift on its own. The
placeholder physics, run-file schema, renderer, and their tests are
this tool's to replace, under its own chain.

`physdemo-check-tool .` (a suite command) verifies that this
repository still satisfies the suite's contract; run it before a
release and after any change to `src/scripts/`, `pyproject.toml`, or
the dependencies.

## Language, Build, and Running

Python 3.10+, NumPy core, vedo/VTK rendering, TOML run files. The
tool reaches a user in two ways that run the same code: **Route A**,
the `physdemo` suite, which owns a shared environment and links
`src/scripts/rfsim.py` (for a shared computer, and for development
here: `sdemo` sources the suite's `activate.sh`); and **Route B**,
`pip install` of this repository, which creates `rfsim` from
`pyproject.toml` (for a laptop, including Windows).
...
(`tests/unit/test_installed_copy.py`; the suite's contract):

- **Everything a run needs is inside `src/rotating_frame/`,** because that
...
- **Offscreen drawing decides VTK's window class before VTK is
  imported** (`render/offscreen.py`), so nothing in `cli/` imports
  the renderer at module level.
...
place**: `__version__` in `src/rotating_frame/__init__.py`. `pyproject.toml`
...
2. Run the test suite, and `physdemo-check-tool .`.
...
**Reading the version.** `rotating_frame.__version__` at run time;
`pip show rotating_frame` for an installed copy.
...
(`tests/conftest.py`) and `rfsim --offscreen` ask VTK for its EGL
...
`src/rotating_frame/render/offscreen.py`. Where no context exists the tests
skip. VTK's import is slow on a shared filesystem, so expect the
first render test to take a minute.
